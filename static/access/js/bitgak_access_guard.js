(function () {
  ...
  function openAccessModal(kind, options) {
    ...
    if (!isLoggedIn() || kind === "login") {
      badge.textContent = options.badge || "무료 회원";
      icon.textContent = options.icon || "🔐";
      title.textContent = options.title || "로그인 후 사용할 수 있습니다";
      text.textContent = options.message || "비로그인 상태에서는 종목 검색, 차트 보기, 드로잉 체험만 가능합니다. 지표 적용과 관심종목 저장은 로그인 후 사용할 수 있습니다.";
      primary.textContent = options.primaryText || "무료 로그인";
      primary.href = options.href || getLoginUrl();
      setPoints(options.points || ["차트 검색 가능", "드로잉 체험 가능", "지표·관심종목은 로그인 필요", "평단가·포트폴리오는 프리미엄"]);
    } else if (kind === "premium") {
      badge.textContent = options.badge || "프리미엄";
      icon.textContent = options.icon || "✨";
      title.textContent = options.title || "프리미엄 기능입니다";
      text.textContent = options.message || "평단가 계산기와 포트폴리오는 프리미엄 회원 또는 쿠폰 이용자만 사용할 수 있습니다.";
      primary.textContent = options.primaryText || "가격소개 보기";
      primary.href = options.href || "/stocks/pricing/";
      if (couponBox && options.showCoupon !== false) {
        couponBox.classList.add("is-visible");
      }
      setPoints(options.points || ["평단가 저장", "포트폴리오 반영", "매수·매도 기록", "프리미엄 쿠폰 사용 가능"]);
    } else {
      badge.textContent = options.badge || "무료 제한";
      icon.textContent = options.icon || "⚡";
      ...
      setPoints(options.points || ["지표 2개까지 무료", "관심종목 10개까지 무료", "종목 그룹 1개 무료", "프리미엄은 제한 해제"]);
    }
