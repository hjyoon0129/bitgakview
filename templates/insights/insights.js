(function () {
  ...
  /* ---------------------------------------------------------
     Insight drawing anchor migration
     - 날짜가 있는 드로잉 점은 logical index가 아니라 시간/날짜를 기준으로 복원한다.
     - 미래 빈 공간처럼 시간값이 없는 점만 logical anchor를 유지한다.
     --------------------------------------------------------- */
  function insightDateKey(value) {
    ...
    if (typeof value === "number" && Number.isFinite(value)) {
      try {
        var ms = Math.abs(value) < 100000000000 ? value * 1000 : value;
        var parts = new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Seoul", year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(new Date(ms));
        var out = {};
  ...
  function normalizeInsightAnchorValue(value) {
    if (!value || typeof value !== "object" || Array.isArray(value)) return value;
    var fixed = Object.assign({}, value);
    var timeValue = fixed.source_time || fixed.display_time || fixed.date_key || fixed.time;
    if (timeValue !== null && timeValue !== undefined && timeValue !== "") {
      fixed.anchor_mode = "time";
      if (!fixed.source_time) fixed.source_time = fixed.display_time || fixed.time;
      if (!fixed.display_time && typeof fixed.source_time === "string") fixed.display_time = fixed.source_time;
      if (!fixed.date_key) fixed.date_key = insightDateKey(fixed.source_time || fixed.display_time || fixed.time);
    } else if (fixed.logical !== null && fixed.logical !== undefined && Number.isFinite(Number(fixed.logical))) {
      fixed.anchor_mode = "logical";
    }
    return fixed;
  }

  function normalizeInsightDrawing(drawing) {
    if (!drawing || typeof drawing !== "object") return drawing;
    var fixed = Object.assign({}, drawing);
    ["start", "end", "point", "from", "to", "p1", "p2", "center"].forEach(function (key) {
      if (fixed[key] && typeof fixed[key] === "object") fixed[key] = normalizeInsightAnchorValue(fixed[key]);
    });
    ["points", "anchors", "vertices"].forEach(function (key) {
      if (Array.isArray(fixed[key])) fixed[key] = fixed[key].map(normalizeInsightAnchorValue);
    });
    fixed.anchor_version = Math.max(2, Number(fixed.anchor_version || fixed.anchorVersion || 0));
    return fixed;
  }

  function normalizeInsightSnapshotAnchors(snapshot, reason) {
    if (!snapshot || typeof snapshot !== "object") return snapshot;
    var fixed = Object.assign({}, snapshot);
    if (Array.isArray(fixed.drawings)) fixed.drawings = fixed.drawings.map(normalizeInsightDrawing);
    fixed.drawingAnchorVersion = 2;
    fixed.drawingAnchorMode = "date-first";
    if (reason) fixed.drawingAnchorNormalizedReason = reason;
    return fixed;
  }
  ...
  function normalizeDraftPayload(data) {
    ...
    return {
      ...
      chart_snapshot: (function () {
        var raw = typeof draft.chart_snapshot === "string" ? safeJsonParse(draft.chart_snapshot, {}) : (draft.chart_snapshot || draft.snapshot || {});
        return toJson(normalizeInsightSnapshotAnchors(raw, "normalize-draft-payload"));
      })()
    };
  }
  ...
  function insightFrameIntervalPreserveScript() {
    return `(function(){
      "use strict";
      if (window.__BITGAK_INSIGHT_FRAME_BRIDGE_V13__) return;
      window.__BITGAK_INSIGHT_FRAME_BRIDGE_V13__ = true;
      ...
      function normalizeAnchorValue(value) {
        ...
        if (timeValue !== null && timeValue !== undefined && timeValue !== "") {
          fixed.anchor_mode = "time";
          if (!fixed.source_time) fixed.source_time = fixed.display_time || fixed.time;
          if (!fixed.date_key) fixed.date_key = dateKey(fixed.source_time || fixed.display_time || fixed.time);
        } else if (fixed.logical !== null && fixed.logical !== undefined && Number.isFinite(Number(fixed.logical))) {
          fixed.anchor_mode = "logical";
        }
        return fixed;
      ...
      function normalizeDrawing(drawing) {
        ...
        var fixed = Object.assign({}, drawing);
        ["start", "end", "point", "from", "to", "p1", "p2", "center"].forEach(function (key) {
          if (fixed[key] && typeof fixed[key] === "object") fixed[key] = normalizeAnchorValue(fixed[key]);
        });
        ["points", "anchors", "vertices"].forEach(function (key) {
          if (Array.isArray(fixed[key])) fixed[key] = fixed[key].map(normalizeAnchorValue);
        });
        fixed.anchor_version = Math.max(2, Number(fixed.anchor_version || fixed.anchorVersion || 0));
        return fixed;
      }

      function normalizeSnapshotAnchors(snapshot, reason) {
        if (!snapshot || typeof snapshot !== "object") return snapshot;
        var fixed = Object.assign({}, snapshot);
        if (Array.isArray(fixed.drawings)) fixed.drawings = fixed.drawings.map(normalizeDrawing);
        fixed.drawingAnchorVersion = 2;
        fixed.drawingAnchorMode = "date-first";
        if (reason) fixed.drawingAnchorNormalizedReason = reason;
        return fixed;
      ...
      function protectIntervalDrawings(snapshot, reason) {
        if (!snapshot || typeof snapshot !== "object") return snapshot;
        snapshot = normalizeSnapshotAnchors(snapshot, reason || "protect-interval");
      ...
      function stripViewport(snapshot) {
        ...
        fixed.viewportStrippedReason = "insight-frame-bridge-v13";
        return fixed;
      }
      ...
      function applyInsightSnapshotToChart(rawSnapshot, reason) {
        ...
        var snapshot = normalizeSnapshotAnchors(stripViewport(clone(rawSnapshot) || rawSnapshot), reason || "apply-snapshot");
      ...
      function scheduleSnapshotApply(rawSnapshot, reason) {
        ...
        pendingInsightSnapshot = normalizeSnapshotAnchors(stripViewport(clone(rawSnapshot) || rawSnapshot), reason || "schedule-snapshot");
      ...
      function captureSnapshot(intervalOverride) {
        ...
        snapshot.source = "bitgakview-insight-frame-bridge-v13";
        ...
        snapshot = normalizeSnapshotAnchors(snapshot, "capture-snapshot");
  ...
  function stripInsightViewportOnly(snapshot, reason) {
    if (!snapshot || typeof snapshot !== "object") return snapshot;
    var fixed = normalizeInsightSnapshotAnchors(Object.assign({}, snapshot), reason || "strip-viewport");
